...
export const WhatWeDo = () => {
  ...
  return (
    ...
    <h2 className=" w-fit text-start text-4xl text-white capitalize mt-6 mx-auto">
    From Idea to Impact
      </h2>
    <p className="text-zinc-400 text-xs max-w-[500px]  mt-3  text-center font-sans mx-auto">
    Whether it’s web, mobile, or automation — we handle the full stack so you can focus on growth.
      </p>
      <div className="siteContainer grid grid-cols-1 lg:grid-cols-2 gap-10  md:gap-4 lg:gap-20 mt-6">
        <article className="flex flex-col gap-12">
          <div>
          <h3 className=" max-w-[400px] text-start text-xl text-white capitalize mt-6">
        What We Do ?
      </h3>
      <p className="text-zinc-400 text-md max-w-[500px]  mt-3  text-start font-sans">
        We create digital solutions that help your business grow. From software
        to marketing,
        we have you covered.
      </p>
          </div>
          ...
          <Motion
            ...
            className=" relative  rounded-3xl overflow-hidden  w-full aspect-video text-white"
          >
            <Image
              fill
              alt="what we do"
        ...
        <div className="space-y-1 flex flex-col justify-between">
          {whatWeDo.map((el, index) => (
            <div
              ...
              key={el.label}
              className={cn(
                "min-h-[100px] border border-zinc-700 rounded-lg p-5 cursor-pointer transition",
                index === theImage && "border-gradient_main-start"
              )}
            >
              <div className="overflow-hidden">
                <WhatWeDoCard whatWeDo={el} />
              </div>
            </div>
          ))}
        </div>
